...
export const decryptMnemonics = async (pass, setWrongPass, setNavigate) => {
    ...
    // key generation 
    pbkdf2(pass, 'this is just a randome string ', 100000, 32, 'sha512', async(err, derivedKey) => {
        if (err) throw err;
        console.log("derived key : ", derivedKey.toString('hex'));  // '3745e48...08d59ae'
        // decryptMnemonics
...
export const getDetails = async()=>{
    ...
    const ballance = await Web3.utils.fromWei(await web3.eth.getBalance(address), 'ether')
